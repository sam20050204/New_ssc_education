...
class SalesReceipt(models.Model):
    """Saved bills/receipts for sold items and services."""
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            original = SalesReceipt.objects.get(pk=self.pk)
            if self.receipt_no != original.receipt_no:
                self.receipt_no = original.receipt_no
        ...
        if not self.receipt_no:
            with transaction.atomic():
                last_receipt = (
                    ...
                    .filter(receipt_no__startswith="SAL-")
                    .order_by("-created_at")
                    .first()
                )
                ...
                if last_receipt and last_receipt.receipt_no:
                    try:
                        ...
                        new_number = SalesReceipt.objects.filter(receipt_no__startswith="SAL-").count() + 1
                else:
                    new_number = 1

                self.receipt_no = f"SAL-{new_number:06d}"
